...
<div class="float-container">
	...
	<div class="left">
		<div class="row">
			<h2 class="library-areaName" style="display:inline-block;"><%=areaList.get(0).getAreaName()%></h2>
		</div>
		<div class="row">
			<h3>- 지점 위치</h3>
			<span><%=areaList.get(0).getAreaLocation()%></span>
		</div>
		
		<div class="row">
			<h3>- 전화번호</h3>
			<span><%=areaList.get(0).getAreaCall()%></span>
		</div>
...
<script>
	...
	geocoder.addressSearch('<%=areaList.get(0).getAreaLocation()%>', function(result, status) {
	     ...
	     if (status === kakao.maps.services.Status.OK) {
	        ...
	        // 인포윈도우로 장소에 대한 설명을 표시합니다
	        var infowindow = new kakao.maps.InfoWindow({
	            content: '<div style="width:150px;text-align:center;padding:6px 0;"><%=areaList.get(0).getAreaName()%></div>'
	        });
